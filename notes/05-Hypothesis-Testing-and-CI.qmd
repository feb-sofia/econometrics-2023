...
## OLS Estimator in the Simple Regression Case
...
Now we can notice that $\hat{\beta}_0$ and $\hat{\beta}_1$ do not depend on the index of the sums, so we can 
write $\hat{\beta}_1$ in front of the sum.
...
\overline{y} - \hat{\beta}_0 - \hat{\beta}_1 \overline{x} = 0 \implies \\
\end{align}
$$

Finally we bring all term except $\overline{y}$ to the right hand side of the equation to obtain:

$$
\overline{y} = \hat{\beta}_0 + \hat{\beta}_1 \overline{x}
$$
This equation informs us that the OLS regression line passes through the center of the observations: $(\overline{x}, \overline{y})$.
...
As with the previous derivative we simplify the expression by deviding by -2 and expanding
...
\end{align}
$$
An easy way to solve the system of equations is to multiply the first equation by $\overlin{x}$ and subtract the first equation from the second one. This
will eliminate $\hat{\beta}_0$ and allow us to solve for $\hat{\beta}_1$.

$$
\begin{align}
\overline{xy} - \overline{x}\overline{y} = \hat{\beta}_0 \overline{x} + \hat{\beta}_1 \overline{x^2} - \hat{\beta}_0 \overline{x} - \hat{\beta}_1 \overline{x}^2 \\
\overline{xy} - \overline{x}\overline{y} = \hat{\beta}_1 \overline{x^2}- \hat{\beta}_1 \overline{x}^2 \\
\overline{xy} - \overline{x}\overline{y} = \hat{\beta}_1 (\overline{x^2} - \overline{x}^2) \implies \\
\hat{\beta}_1 = \frac{\overline{xy} - \overline{x}\overline{y}}{\overline{x^2} - \overline{x}^2}
\end{align}
$$
So finally we obtain the whole solution:
...
In the case when $x_i \in \{0, 1\}$ the average of $x$, i.e $\overline{x}$ is simply the proportion of ones, lets say $n_1 / n$. Notice that the average of the squared $x$ is the same as the average of $x$, because $x$ only contains zeroes and ones. The average of the product of $x$ and $y$ is equal to the sum of $y$ where the $x = 1$, because the other values in $y$ are multiplied by zero.

$$
\overline{x^2} = \overline{x} \\
\overline{xy} = \frac{1}{n}\sum_{i: x_i = 1} y_i = \frac{n_1}{n_1}\frac{1}{n} \sum_{i: x_i = 1} y_i  = \frac{n_1}{n} \overline{y}_{1}
$$ 
...
Furthermore the average of $y$ is simply the weighted average of the two group means. Let there be $n_0$ observations with 
$x_i = 0$ and $n_1$ observations with $x_i = 1$.
...
$$
\hat{\beta}_1 = \overline{y}_1 - \overline{y}_0
$$
From the equation for $\hat{\beta}_0$ we get:

$$
\hat{\beta}_0 = \overline{y} - (\overline{y}_1 - \overline{y}_0) \overline{x}
$$
Taking into account that $\overline{x} = n_1 / n$ this equation simplifies to
