...
In the previous class we fitted a linear regression model for the IQ score of
the children in the `kids` dataset depending on the educational status of their mother `mom_hs`.
...
Now we would like to test whether there is a difference between the two groups of children in the _population_ (we already know
that there is a difference in the sample!).
...
$$
$$
\text{t} = \frac{\hat{\beta}_1 - ?}{se(\hat{\beta}_1)}
$$
...
3. Make a decision to reject $H_0$ if the t-statistic is less than -2. In how many 
 cases you make a wrong decision?


```{r}
slopes <- slopes %>%
  mutate(
    ## Logical OR: |, logical comparison LESS THAN: <
    reject_H0 = 
  )
...
1. Compute the value of the t-statistic for all samples in the simulation
...
3. In how many cases would you make a wrong decision if you reject $H_0$ for values of the t-statistic les than -2?
...
## Probabilities using the t-distribution
...
The p-value is the probability to observe (by chance) a t-statistic more extreme (i.e. against the null hypothesis) than the one observed in 
the sample assuming that $H_0$ is true. For the one-sided hypothesis considered here it is given by

$$
\text{p-value} = P(t < t^{obs} | H_0)
$$
Compute the p-value for
