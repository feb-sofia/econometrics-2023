...
The coefficient of U is positive. This implies that expected crime rate $\hat{C}$ increases with increasing values of $U$ (degree of urbanisation).
...
When is the expected crime rate equal to to the intercept (52.4)? =\> When $\text{HS}_i - \overline{HS} = 0$, i.e. it estimates the expected crime for a county with an average share of persons with a school degree ($\text{HS} = \overline{HS}$)
...
For a given level of urbanisation, the number of crimes does not vary systematically with the share of residents with a school degree.
